<template>
  <div class="mapItem">
    <b-row>
      <b-col cols="5" class="scrollHeight searchMenu">
        <search-bar class="searchBar" :data="dataSource" @updated="updated" />
      </b-col>
      <b-col cols="12" class="scrollHeight" v-if="isMobile">
        <div class="w-100 map" id="map"></div>
      </b-col>
      <b-col cols="7" class="scrollHeight" v-else>
        <div class="w-100 map" id="map"></div>
      </b-col>
    </b-row>
...
export default {
  ...
  data() {
    return {
      masksURL:
        "https://raw.githubusercontent.com/kiang/pharmacies/master/json/points.json?fbclid=IwAR1_UoJFvXnQz73Z2gLR0ZbFK2zVsw8Uf6l2hwtmZQM48YA8UpkPx4PSCyA",
      zoom: settings.zoom,
      url: settings.url,
  ...
  methods: {
    getPopup(item) {
      let phone = `(${item.phone.substr(0, 2)}) ${item.phone.substr(
        4,
        4
      )}-${item.phone.substr(8)}`;
      return `
        ...
        <div class="storeInfo">
          <div><i class="iconMap mapIcon"></i> <a target="_blank" href="https://www.google.com.tw/maps/place/${
            item.address
          }">${item.address}</a></div>
          <div><i class="iconMap phoneIcon"></i> <a href="tel:+886-${item.phone
            .substring(1)
            .replace(" ", "")}">${phone}</a></div>
          <hr>
          <div class="maskStock">
          ...
          <div class="updateTime">更新時間: ${
            item.updated === "" ? "(不明)" : item.updated
          }</div>
        </div>
      `;
    },
    handleCustomIcon(markers, feature, latLng) {
      ...
      markers.addLayer(
        L.marker(latLng, { icon: rangeIcon }).bindPopup(
          this.getPopup(feature.properties)
        )
      );
    ...
    handleMap(data) {
      ...
      for (let i = 0; data.length > i; i++) {
        this.handleCustomIcon(markers, data[i], [
      ...
      if (navigator.geolocation) {
        navigator.geolocation.getCurrentPosition(
          ...
          function (position) {
            ...
            vm.zoom = 19;
            map.setView(
              new L.LatLng(vm.userPosition[0], vm.userPosition[1]),
              vm.zoom
            );
            vm.userMarker = L.marker([vm.userPosition[0], vm.userPosition[1]], {
              icon: vm.customIcon("red"),
            });
